...
namespace RuntimeNodeEditor
{
    public class NodeGraph : MonoBehaviour
    {
        ...
        {
            strelka_pos.me.vibron.GetComponent<scriptor>().nodesCode = ExportJson();
        }

        public void LoadFile(string json)
        {
            if (json != null)
            {
                try
                {
                    ...
                    foreach (var node in nodes)
                    {
                        ...
                        for (int i = 0; i < nodeData.inputSocketIds.Count; i++)
                        {
                            node.Inputs[i].socketId = nodeData.inputSocketIds[i];
                        }

                        for (int i = 0; i < nodeData.outputSocketIds.Count; i++)
                        {
        ...
        public GraphData Export()
        {
            ...
            foreach (var node in nodes)
            {
                ...
                data.inputSocketIds = inputIds.ToArray().ToList();
                data.outputSocketIds = outputIds.ToArray().ToList();
        ...
        protected virtual void OnGraphPointerScrolled(PointerEventData eventData)
        {
            ...
            {
                // Добавляем плавность и предотвращаем резкие изменения при достижении границ
                float zoomFactor = Mathf.Sign(eventData.scrollDelta.y);

                // Если мы уже у границы и пытаемся выйти за неё - игнорируем ввод
                if ((zoomFactor > 0 && Mathf.Approximately(_currentZoom, _maxZoom)) ||
                    (zoomFactor < 0 && Mathf.Approximately(_currentZoom, _minZoom)))
                {
                    return;
                }

                // Изменяем масштаб с плавностью и ограничением скорости
                float zoomSpeed = 0.1f; // Можно настроить под ваши нужды
                _currentZoom *= 1f + zoomFactor * zoomSpeed;
